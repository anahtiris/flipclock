...
function getRange(type: UnitType, year: number, month: number, hour12: boolean) {
  switch (type) {
    ...
    default: {
      ...
      throw new Error(`Unhandled UnitType: ${_exhaustive}`)
    }
  }
}
...
export function UnitGroup({
  ...
  const pad = value.toString().padStart(2, '0')
  const tens = parseInt(pad[0])
  const units = parseInt(pad[1])

  const handleDigitScroll = useCallback((digitIndex: number, dir: FlipDirection) => {
    flipDirRef.current = (-dir) as FlipDirection
    const { min, max } = getRange(type, year, month, hour12)

    if (scrollMode === 'unit') {
      let next = value + dir
      if (next > max) { next = min; onCarry?.(1) }
      else if (next < min) { next = max; onCarry?.(-1) }
      onChange(next)
      return
    }
  ...
  }, [value, tens, units, type, year, month, hour12, scrollMode, onChange, onCarry])
  ...
  return (
    <>
      <FlipCard value={pad[0]} theme={theme} size={size} dir={flipDirRef.current}
        onScroll={handleTensScroll} />
      <FlipCard value={pad[1]} theme={theme} size={size} dir={flipDirRef.current}
